document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DOM Cache (Initialized in initApp) ---
    let dom = {};
    let ctx;
    ...
    const onInteractionStart = (e) => {
        ...
        // Prioritize the top layer (accent) for selection
        ...
        if (isDragging || isResizing) {
            e.preventDefault();
            dragStart = pos;
            dom.canvas.classList.add(isResizing ? 'resizing' : 'grabbing');
        }
        drawCanvas();
    };

    const onInteractionMove = (e) => {
        if (!imagesLoaded || (!isDragging && !isResizing)) {
            const pos = getMousePos(e);
            dom.canvas.style.cursor = (selectedLayer && selectedLayer.bbox.handle && isInsideRect(pos, selectedLayer.bbox.handle)) ? 'nwse-resize' : 'grab';
            return;
        }
        e.preventDefault();
        const pos = getMousePos(e);
    ...
    const onInteractionEnd = () => {
        isDragging = false;
        isResizing = false;
        dom.canvas.classList.remove('grabbing', 'resizing');
    };

    const setupEventListeners = (container) => {
        const C = cls => container.querySelector(cls);
        const setup = (el, path, event = 'input') => el?.addEventListener(event, () => {
            let v = el.type === 'checkbox' ? (el.checked ? 'multi' : 'single') : el.type === 'range' ? parseFloat(el.value) : el.value;
            const k = path.split('.');
            let s = state;
            for (let i = 0; i < k.length - 1; i++) s = s[k[i]];
            s[k[k.length - 1]] = v;
            drawCanvas();
        });

        setup(C('.control-behind-text'), 'textLayers.main.text');
        setup(C('.control-front-text'), 'textLayers.accent.text');
        setup(C('.control-behind-size'), 'textLayers.main.size');
        setup(C('.control-front-size'), 'textLayers.accent.size');
        ...
        const downloadHandler = e => {
            if (!imagesLoaded) {
                e.preventDefault();
                return showToast('Please create an image first.');
            }
            selectedLayer = null;
            drawCanvas(true);
        ...
        // Attach to both desktop and mobile download buttons if they exist
    ...
    function initApp() {
        ...
        if (!dom.canvas) {
            console.error("FATAL ERROR: Canvas element with ID 'previewCanvas' was not found.");
            return; 
        }
        ctx = dom.canvas.getContext('2d');
        
        ['text', 'style', 'export'].forEach(type => {
            const template = dom.controlTemplates.querySelector(`[data-template="${type}"]`);
            if (template) dom.desktopSidebar.append(template.cloneNode(true));
        });
